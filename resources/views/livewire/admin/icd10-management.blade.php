<div>
    <!-- Breadcrumb -->
    <x-breadcrumb :items="[['label' => 'Admin', 'url' => '#'], ['label' => 'Master ICD-10']]" />

    <!-- Header & Search Section -->
    <div class="mb-6 px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">📋 Master ICD-10</h2>
                <p class="text-sm text-gray-500 mt-1">Kelola kode diagnosis ICD-10 untuk sistem kesehatan</p>
            </div>
            <button wire:click="create"
                ...
                <span class="hidden sm:inline">Tambah Kode ICD-10</span>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 px-4 sm:px-6 lg:px-8">
        <button wire:click="$set('statusFilter', 'all')"
            ...
            <p class="text-xs md:text-sm font-medium text-gray-600">Total Kode</p>
        ...
        <button wire:click="$set('statusFilter', 'active')"
            ...
            <p class="text-sm font-medium text-gray-600">Kode Aktif</p>
        ...
        <button wire:click="$set('statusFilter', 'inactive')"
            ...
            <p class="text-sm font-medium text-gray-600">Kode Nonaktif</p>
        ...
        <button wire:click="$set('statusFilter', 'deleted')"
            ...
            <p class="text-sm font-medium text-gray-600">Kode Terhapus</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div class="md:col-span-3">
                <select wire:model.live="categoryFilter"
                    ...
                    <option value="A00-B99">Infeksi & Parasit</option>
                    <option value="C00-D48">Neoplasma</option>
                    <option value="D50-D89">Darah & Sistem Imun</option>
                    <option value="E00-E90">Endokrin & Metabolik</option>
                    <option value="F00-F99">Mental & Perilaku</option>
                    <option value="G00-G99">Sistem Saraf</option>
                    <option value="H00-H59">Mata & Adneksa</option>
                    <option value="H60-H95">Telinga & Mastoid</option>
                    <option value="I00-I99">Sirkulasi</option>
                    <option value="J00-J99">Respirasi</option>
                    <option value="K00-K93">Digestif</option>
                    <option value="L00-L99">Kulit & Subkutan</option>
                    <option value="M00-M99">Muskuloskeletal</option>
                    <option value="N00-N99">Genitourinari</option>
                    <option value="O00-O99">Kehamilan & Persalinan</option>
                    <option value="P00-P96">Perinatal</option>
                    <option value="Q00-Q99">Kongenital</option>
                    <option value="R00-R99">Simptom & Tanda</option>
                    <option value="S00-T98">Cedera & Keracunan</option>
                    <option value="V01-Y98">Eksternal</option>
                    <option value="Z00-Z99">Faktor yang Mempengaruhi Kesehatan</option>
                </select>
            </div>
    ...
    <div wire:loading class="mb-4 px-4 sm:px-6 lg:px-8">
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 flex items-center gap-3">
            ...
            <span class="text-sm text-blue-700 font-medium">Memuat data ICD-10...</span>
        </div>
    </div>
                    ...
                    <tr wire:loading.delay>
                        <td colspan="6" class="px-6 py-8 bg-gray-50">
                            <div class="flex items-center justify-center space-x-3">
                                ...
                                <span class="text-sm text-gray-600">Memuat data ICD-10...</span>
                            </div>
                        </td>
                    </tr>
                    ...
                    @forelse($codes as $code)
                        <tr class="hover:bg-blue-50 transition-colors" wire:loading.remove>
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if ($code->description)
                                    <div class="text-sm text-gray-500 mt-1">{{ Str::limit($code->description, 60) }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $code->category }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-700">
                                    @if (is_array($code->keywords) && count($code->keywords) > 0)
                                        <div class="flex flex-wrap gap-1">
                                            @foreach (array_slice($code->keywords, 0, 3) as $keyword)
                                                <span
                                                    class="inline-flex items-center px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                                                    {{ $keyword }}
                                                </span>
                                            @endforeach
                                            @if (count($code->keywords) > 3)
                                                <span class="text-xs text-gray-500">+{{ count($code->keywords) - 3 }}
                                                    lagi</span>
                                            @endif
                                        </div>
                                    @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    ...
                                    @else
                                        <button wire:click="restore({{ $code->id }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <p class="text-gray-500 font-medium mb-2">Tidak ada data ICD-10 ditemukan</p>
                                @if ($search || $statusFilter !== 'all' || $categoryFilter)
                                    <button wire:click="clearFilters"
                                        class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                                        Reset filter untuk melihat semua kode ICD-10
                                    </button>
                                @else
                                    <button wire:click="create"
                                        class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                                        Tambah kode ICD-10 pertama →
                                    </button>
                                @endif
                            </td>
                        </tr>
    ...
    <div class="mt-4 text-sm text-gray-500 text-center px-4 sm:px-6 lg:px-8">
        Menampilkan {{ $codes->firstItem() ?? 0 }} - {{ $codes->lastItem() ?? 0 }} dari
        {{ $codes->total() }} kode ICD-10
    </div>
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 w-full max-w-2xl">
                <h3 class="text-lg font-bold mb-4">{{ $editing ? 'Edit ICD' : 'Tambah ICD' }}</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input wire:model.defer="category" class="w-full px-3 py-2 border rounded-lg">
                    </div>
                    <div class="md:col-span-2">
                        <label class="text-sm font-semibold">Keywords (pisahkan dengan koma)</label>
                        <input wire:model.defer="keywords" class="w-full px-3 py-2 border rounded-lg">
                    </div>
                    <div>
