<div class="space-y-6">
    ...
    <!-- Search & Selection -->
    <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        @if (!$selectedPatient && !$selectedChild)
            <div class="space-y-4">
                <div class="relative">
                    <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Cari Pasien / Anak</label>
                    <div class="relative">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" id="search"
                            ...
                            placeholder="Cari Nama, NIK, atau No. RM (Pasien/Anak)..." autofocus>
                    </div>
                </div>
                ...
                @if (!empty($results) && count($results) > 0)
                    <div class="mb-4">
                        <h4 class="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                            <x-heroicon-o-user class="h-4 w-4 text-blue-500" />
                ...
                {{-- Child Results --}}
                @if (!empty($childResults) && count($childResults) > 0)
                    <div class="mb-4">
                        <h4 class="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                            <x-heroicon-o-face-smile class="h-4 w-4 text-green-500" />
                                    ...
                                    @foreach ($childResults as $child)
                                        <tr wire:key="child-{{ $child->id }}" class="hover:bg-green-50 cursor-pointer" wire:click="selectChild({{ $child->id }})">
                                            ...
                                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-700">
                                                {{ $child->name }}</td>
                                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500">
                                                {{ $child->formatted_age ?? '-' }}</td>
                                            <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-500">
                                                {{ $child->parent_display_name }}</td>
                                            <td class="px-4 py-2 whitespace-nowrap text-right text-sm font-medium">
                                                <button wire:click="selectChild({{ $child->id }})"
                                                    class="text-green-600 hover:text-green-900">Pilih</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                ...
                {{-- No Results --}}
                @if(strlen($search) >= 2 && empty($results) && empty($childResults))
                    <div class="text-center py-4 text-gray-500">
                        Tidak ditemukan.
                        <a href="{{ route('patients.create') }}" class="text-blue-600 font-medium hover:underline">Buat Pasien Baru</a>
                        atau
                        <a href="{{ route('children.register') }}" class="text-green-600 font-medium hover:underline">Daftarkan Anak Baru</a>
                    </div>
                @endif

                <div class="mt-4 pt-4 border-t border-gray-100 flex justify-end gap-3">
                    <!-- Shortcut: Direct Child Registration (External Birth) -->
                    <a href="{{ route('children.register') }}"
                        ...
                        <x-heroicon-o-face-smile class="h-5 w-5 mr-2" />
                        Daftarkan Anak Baru
                    </a>
        ...
        @elseif($selectionType === 'patient' && $selectedPatient)
        ...
        @elseif($selectionType === 'child' && $selectedChild)
            <!-- Selected Child View -->
            <div>
                <div class="flex justify-between items-start mb-6">
                    <div>
                        <h3 class="text-lg font-medium leading-6 text-gray-900">Anak Terpilih</h3>
                        <p class="mt-1 max-w-2xl text-sm text-gray-500">Silahkan pilih layanan untuk anak ini.</p>
                    </div>
                    <button wire:click="resetSelection"
                        class="text-sm text-gray-500 hover:text-gray-700 flex items-center">
                        <x-heroicon-o-x-mark class="h-5 w-5 mr-1" />
                ...
                <div class="bg-green-50 border-l-4 border-green-400 p-4 mb-6">
                    <div class="flex">
                        <div class="flex-shrink-0">
                            <x-heroicon-o-face-smile class="h-5 w-5 text-green-400" />
                        </div>
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-green-800">{{ $selectedChild->name }}
                                ({{ $selectedChild->no_rm }})</h3>
                            <div class="mt-2 text-sm text-green-700 grid grid-cols-1 md:grid-cols-2 gap-2">
                                <p><span class="font-medium">Usia:</span> {{ $selectedChild->formatted_age ?? '-' }}</p>
                                <p><span class="font-medium">Tgl Lahir:</span>
                                    {{ $selectedChild->dob ? $selectedChild->dob->format('d/m/Y') : '-' }}</p>
                                <p><span class="font-medium">Jenis Kelamin:</span>
                                    {{ $selectedChild->gender == 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
                                <p><span class="font-medium">Orang Tua:</span> {{ $selectedChild->parent_display_name }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
                    ...
                    <button wire:click="selectChildService('general')"
                        ...
                        <h4 class="font-semibold text-gray-900">Poli Umum</h4>
                        <p class="text-sm text-gray-500 text-center mt-1">Anak sakit, cek kesehatan.</p>
                    </button>

                    <!-- Imunisasi -->
                    <button wire:click="selectChildService('immunization')"
                        class="flex flex-col items-center justify-center p-6 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-green-300 transition-all group">
                        <div class="p-3 bg-green-100 rounded-full group-hover:bg-green-200 mb-3">
