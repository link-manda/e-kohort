        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-gradient-to-br from-blue-600 to-blue-800 rounded-xl shadow-lg p-6 text-white">
                <div class="text-center">
                    <div class="w-24 h-24 bg-white rounded-full flex items-center justify-center text-blue-600 text-4xl font-bold mx-auto mb-4 shadow-lg">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-4 text-sm">
                    ...
                    <div>
                        ...
                        <p class="text-gray-900 font-medium">{{ $patient->dob->locale('id')->isoFormat('D MMMM YYYY') }}</p>
                    </div>
                    <div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <div class="text-center p-4 bg-blue-50 rounded-lg">
                                ...
                                <p class="text-2xl font-bold text-blue-900">{{ $patient->activePregnancy->gravida }}</p>
                            </div>
                            <div class="text-center p-4 bg-purple-50 rounded-lg">
                                <p class="text-xs text-purple-600 font-medium mb-1">Usia Kehamilan</p>
                                <p class="text-2xl font-bold text-purple-900">{{ $patient->activePregnancy->gestational_age }}</p>
                                <p class="text-xs text-purple-600">minggu</p>
                            </div>
                            <div class="text-center p-4 bg-green-50 rounded-lg">
                                <p class="text-xs text-green-600 font-medium mb-1">HPHT</p>
                                <p class="text-sm font-bold text-green-900">{{ $patient->activePregnancy->hpht->format('d/m/Y') }}</p>
                            </div>
                            <div class="text-center p-4 bg-orange-50 rounded-lg">
                                <p class="text-xs text-orange-600 font-medium mb-1">HPL</p>
                                <p class="text-sm font-bold text-orange-900">{{ $patient->activePregnancy->hpl->format('d/m/Y') }}</p>
                            </div>
                        </div>
                        ...
                        @if($latestVisit)
                        <!-- Risk Summary -->
                        <div class="mb-6 p-5 rounded-lg {{ $latestVisit->risk_category === 'Ekstrem' ? 'bg-red-50 border-2 border-red-300' : ($latestVisit->risk_category === 'Tinggi' ? 'bg-orange-50 border-2 border-orange-300' : 'bg-green-50 border-2 border-green-300') }}">
                            <div class="flex items-center justify-between mb-4">
                                <h4 class="font-bold text-gray-900 flex items-center gap-2">
                                    <svg class="w-5 h-5 {{ $latestVisit->risk_category === 'Ekstrem' ? 'text-red-600' : ($latestVisit->risk_category === 'Tinggi' ? 'text-orange-600' : 'text-green-600') }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>
                                    Status Risiko Terkini
                                </h4>
                                <span class="px-4 py-2 text-sm font-bold rounded-full {{ $latestVisit->risk_category === 'Ekstrem' ? 'bg-red-600 text-white' : ($latestVisit->risk_category === 'Tinggi' ? 'bg-orange-500 text-white' : 'bg-green-500 text-white') }}">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                                <div class="bg-white p-3 rounded-lg">
                                    ...
                                    <p class="text-lg font-bold {{ $latestVisit->map_score > 100 ? 'text-red-600' : ($latestVisit->map_score > 90 ? 'text-orange-600' : 'text-green-600') }}">
                                        {{ number_format($latestVisit->map_score, 1) }}
                                    </p>
                                </div>
                                <div class="bg-white p-3 rounded-lg">
                                    <p class="text-xs text-gray-600 mb-1">Hemoglobin</p>
                                    <p class="text-lg font-bold {{ $latestVisit->hb && $latestVisit->hb < 11 ? 'text-red-600' : 'text-green-600' }}">
                                        {{ $latestVisit->hb ? number_format($latestVisit->hb, 1) . ' g/dL' : '-' }}
                                    </p>
                                </div>
                                <div class="bg-white p-3 rounded-lg">
                                    <p class="text-xs text-gray-600 mb-1">LILA</p>
                                    <p class="text-lg font-bold {{ $latestVisit->lila && $latestVisit->lila < 23.5 ? 'text-red-600' : 'text-green-600' }}">
                                        {{ $latestVisit->lila ? number_format($latestVisit->lila, 1) . ' cm' : '-' }}
                                    </p>
                                </div>
                                <div class="bg-white p-3 rounded-lg">
                                    <p class="text-xs text-gray-600 mb-1">HIV</p>
                                    <span class="text-sm font-bold {{ $latestVisit->hiv_status === 'R' ? 'text-red-600' : 'text-green-600' }}">
                                        {{ $latestVisit->hiv_status === 'R' ? 'Reaktif' : 'Non-Reaktif' }}
                                    </span>
                                </div>
                                <div class="bg-white p-3 rounded-lg">
                                    <p class="text-xs text-gray-600 mb-1">Syphilis</p>
                                    <span class="text-sm font-bold {{ $latestVisit->syphilis_status === 'R' ? 'text-red-600' : 'text-green-600' }}">
                                        {{ $latestVisit->syphilis_status === 'R' ? 'Reaktif' : 'Non-Reaktif' }}
                                    </span>
                                </div>
                                <div class="bg-white p-3 rounded-lg">
                                    <p class="text-xs text-gray-600 mb-1">HBsAg</p>
                                    <span class="text-sm font-bold {{ $latestVisit->hbsag_status === 'R' ? 'text-red-600' : 'text-green-600' }}">
                                        {{ $latestVisit->hbsag_status === 'R' ? 'Reaktif' : 'Non-Reaktif' }}
                                    </span>
                                </div>
                            </div>
                            <p class="text-xs text-gray-600 mt-3">Kunjungan terakhir: {{ $latestVisit->visit_date->locale('id')->isoFormat('D MMMM YYYY') }}</p>
                        </div>
                        @endif
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <span class="text-sm text-gray-500">{{ $patient->activePregnancy->ancVisits->count() }} kunjungan</span>
                            </div>

                            @if($patient->activePregnancy->ancVisits->count() > 0)
                                <div class="space-y-3">
                                    @foreach($patient->activePregnancy->ancVisits->sortByDesc('visit_date') as $visit)
                                        <div class="flex items-center gap-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors border border-gray-200">
                                            <div class="w-14 h-14 flex-shrink-0 rounded-lg flex items-center justify-center text-white font-bold text-lg
                                            ...
                                            <div class="flex-1">
                                                <p class="font-semibold text-gray-900">{{ $visit->visit_date->locale('id')->isoFormat('D MMMM YYYY') }}</p>
                                                <p class="text-sm text-gray-600">UK: {{ $visit->gestational_age }} minggu • MAP: {{ number_format($visit->map_score, 1) }}</p>
                                            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-3">
                    @foreach($patient->pregnancies->sortByDesc('created_at') as $pregnancy)
                        <div class="p-4 rounded-lg border-2 {{ $pregnancy->status === 'Aktif' ? 'border-green-300 bg-green-50' : 'border-gray-200 bg-gray-50' }}">
                            <div class="flex items-center justify-between mb-2">
                                <span class="font-semibold text-gray-900">{{ $pregnancy->gravida }}</span>
                                <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $pregnancy->status === 'Aktif' ? 'bg-green-600 text-white' : 'bg-gray-600 text-white' }}">
                                    {{ $pregnancy->status }}
                                </span>
                            </div>
                            <div class="text-sm text-gray-600">
                                <p>HPHT: {{ $pregnancy->hpht->format('d/m/Y') }} • HPL: {{ $pregnancy->hpl->format('d/m/Y') }}</p>
                                @if($pregnancy->ancVisits->count() > 0)
                                    <p class="mt-1">{{ $pregnancy->ancVisits->count() }} kunjungan ANC tercatat</p>
                                @endif
                            </div>
                        </div>
                    @endforeach
