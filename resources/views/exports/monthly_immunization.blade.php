        ...
        <tbody>
            @forelse($visits as $index => $visit)
                @php
                    ...
                    // Format TTL
                    $ttl = ($child->pob ?? '-') . ', ' . ($child->dob ? \Carbon\Carbon::parse($child->dob)->locale('id')->isoFormat('D MMM YYYY') : '-');

                    // Age
                    $age = $child->dob
                        ? \Carbon\Carbon::parse($child->dob)->diffInMonths(\Carbon\Carbon::parse($visit->visit_date)) . ' bln'
                        : '-';

                    // Parents
                    $parentName = $patient ? $patient->name : ($child->mother_name ?? '-');
                    $parentNik = $patient ? $patient->nik : ($child->mother_nik ?? '-');
                    $addressHp = ($patient ? ($patient->address ?? '-') : ($child->address ?? '-')) . ' / ' . ($patient ? ($patient->phone ?? '-') : ($child->parent_phone ?? '-'));
                    ...
                    // Medicine
                    $medicine = $visit->medicine_given ? $visit->medicine_given . ($visit->medicine_dosage ? ' (' . $visit->medicine_dosage . ')' : '') : '-';
